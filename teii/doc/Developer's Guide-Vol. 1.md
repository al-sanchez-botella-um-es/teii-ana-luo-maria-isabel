...
- La lista de directorios predefinida de la instalación Python. Incluirá habitualmente:
  - Librería estándar de python (módulos python).
  - Librería estándar compilada (extensiones C).
  - Paquetes externos (pip).

```bash
$ pyenv activate venv-teii
(venv-teii) $ export PYTHONPATH=/tmp
(venv-teii) $ python3
>>> import sys
>>> sys.path
['', '/tmp', ... , $USER/.pyenv/versions/venv-teii/lib/python3.11/site-packages']
```
...
- [Python Documentation • The `import` system](https://docs.python.org/3.11/reference/import.html)
...
- [Python Documentation • `logging` — Logging facility for Python](https://docs.python.org/3.11/library/logging.html)
- [Python Documentation • Logging HOWTO](https://docs.python.org/3.11/howto/logging.html)
